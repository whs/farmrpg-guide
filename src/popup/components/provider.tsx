...
providerComponent.set(SubmitQuest, (props: {action: SubmitQuest}) => {
	...
	return <div>Submit quest <strong><img class="inline h-[1.5em]" src={image.toString()} /> {props.action.quest.name}</strong></div>;
});

providerComponent.set(FarmPlant, (props: {action: FarmPlant}) => {
	return <div><img class="inline h-[1.5em]" src="https://farmrpg.com/img/items/6137.png" /> Farm <strong><Item item={props.action.output} /></strong> ×{props.action.desired}</div>;
});
...
providerComponent.set(BuyItemStore, (props: {action: BuyItemStore}) => {
	...
	return (
		<div><img class="inline h-[1.5em]" src="https://farmrpg.com/img/items/silver.png" /> Buy <strong><Item item={props.action.item} /></strong> ×{props.action.amount}</div>
	);
});

providerComponent.set(BuySteak, (props: {action: BuySteak}) => {
	return (
		<div><img class="inline h-[1.5em]" src="https://farmrpg.com/img/items/silver.png" /> Buy <strong><img class="inline h-[1.5em]" src="https://farmrpg.com/img/items/steak.png" /> Steak</strong> ×{props.action.amount}</div>
	);
});

providerComponent.set(BuySteakKabob, (props: {action: BuySteakKabob}) => {
	return (
		<div><img class="inline h-[1.5em]" src="https://farmrpg.com/img/items/silver.png" /> Buy <strong><img class="inline h-[1.5em]" src="https://farmrpg.com/img/items/8898.png" /> Steak Kabob</strong> ×{props.action.amount}</div>
	);
});
...
providerComponent.set(CraftItem, (props: {action: CraftItem}) => {
	return (
		<div><img class="inline h-[1.5em]" src="https://farmrpg.com/img/items/5868.png" /> Craft <strong><Item item={props.action.item} /></strong> ×{props.action.craftTimes}</div>
	);
});
